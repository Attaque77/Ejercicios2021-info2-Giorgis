...
rectangulo rectangulo::obtener()const{
	rectangulo u;
	u.cor_x=cor_x;
	u.cor_y=cor_y;
	u.ancho=ancho;
...
//Para utilizarlo en el 'main' como cascada
rectangulo &rectangulo::area(double *_area){
	(*_area)=abs(ancho*largo);
	return *this;
}

//Para utilizarlo en el 'main' como cascada
rectangulo &rectangulo::perimetro(double *_perimetro){
	
	if(ancho<0)
		(*_perimetro)=(ancho*2*(-1)+largo*2);
	else if(largo<0)
		(*_perimetro)=(ancho*2+largo*2*(-1));
	else{
		(*_perimetro)=(ancho*2+largo*2);
	}
	return *this;
}

//Para utilizarse con las funciones 'intersección' y 'detro'
